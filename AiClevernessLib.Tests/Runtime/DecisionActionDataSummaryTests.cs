...
public sealed class DecisionActionDataSummaryTests
{
    ...
    [Fact]
    public void Create_DeduplicatesTypesAfterTruncation()
    {
        // Arrange
        var sharedPrefix = new string('t', 79);
        var data = new[]
        {
            CreateData(sharedPrefix + "-first", "first"),
            CreateData(sharedPrefix + "-second", "second"),
            CreateData("third-type", "third")
        };
        ...
        // Assert
        summary.Should().NotBeNull();
        summary!.Types.Should().Equal(sharedPrefix + "…", "third-type");
    }
    ...
    [Fact]
    public void Constructor_BoundsTypesAndContentPreviews()
    {
        // Arrange
        ...
        // Act
        var summary = new DecisionActionDataSummary(
            12,
            Enumerable.Repeat(longType, 12).ToArray(),
            Enumerable.Repeat(longPreview, 7).ToArray());

        // Assert
        summary.ItemCount.Should().Be(12);
        summary.Types.Should().HaveCount(10);
        summary.Types.Should().OnlyContain(type => type == new string('t', 79) + "…");
        summary.ContentPreviews.Should().HaveCount(5);
        summary.ContentPreviews.Should().OnlyContain(preview => preview == new string('x', 79) + "…");
    }

    [Fact]
    public void WithExpression_BoundsReplacementTypesAndContentPreviews()
    {
        // Arrange
        var longType = new string('t', 100);
        var longPreview = new string('x', 100);
        ...
        var updated = summary with
        {
            Types = Enumerable.Repeat(longType, 12).ToArray(),
            ContentPreviews = Enumerable.Repeat(longPreview, 7).ToArray()
        };

        // Assert
        updated.Types.Should().HaveCount(10);
        updated.Types.Should().OnlyContain(type => type == new string('t', 79) + "…");
        updated.ContentPreviews.Should().HaveCount(5);
        updated.ContentPreviews.Should().OnlyContain(preview => preview == new string('x', 79) + "…");
    }
