...
│   ├── AgentRuntime.cs    # The orchestrator (middleware pipeline + streaming)
│   ├── DefaultToolExecutor.cs
│   ├── IdempotentToolExecutor.cs
│   ├── ToolCallBuffer.cs
│   ├── DefaultPlanner.cs / SequentialPlanner.cs
│   ├── PlannerRegistry.cs / StrategyRegistry.cs
│   ├── DefaultExecutionContext.cs
│   ├── InMemoryAgentMemory.cs / InMemoryIdempotencyCache.cs
│   ├── InMemoryCheckpointStore.cs / InMemoryExecutionJournal.cs
│   ├── DefaultShutdownCoordinator.cs
│   ├── DefaultMetricsCollector.cs / DefaultDiagnosticCollector.cs
│   ├── DefaultStartupAnalyzer.cs
│   ├── HostedAgentRuntimeService.cs
│   ├── ToolRegistry.cs
│   ├── Capabilities/      # Capability resolution and model selection
│   ├── Conversation/      # Prompt templates, truncation, conversation management
│   ├── Filtering/         # Agent-scoped wrappers (FilteredPolicy, FilteredGate, etc.)
│   ├── Memory/            # Tiered memory (Working, LongTerm, Vector, Aggregate)
│   ├── Middleware/        # Pipeline middleware (Policy, InputValidation, Planning, etc.)
│   ├── Security/          # Approval, scope validation, danger-level checks
│   ├── Strategies/        # Deterministic strategies (CachedResult, RuleBased)
│   └── Workflows/         # Workflow executors and multi-agent patterns
├── DependencyInjection/   # DI extension methods
├── AiClevernessLib.csproj
└── readme.md
```
